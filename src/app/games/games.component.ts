...
export class GamesComponent implements OnInit {
  ...
  getPageItems(): any[] {
    ...
    return this.listGames.slice(startIndex, endIndex);
  }

  getTotalPages(): number {
    return Math.ceil(this.listGames.length / this.itemsPerPage);
  }
  ...
  openConfirmDialog(id: any): void {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        ...
        this.http
          .delete<any>(
            'https://cdtkpmnc2023final.azurewebsites.net/api/Game/' + id
          )
          .subscribe((res) => {
            console.log(res);
            location.reload();
  ...
  openEditGameDialog(id: any): void {
    const dialogConfig: MatDialogConfig = {
      width: '800px',
      position: {
        left: '350px',
    ...
    const dialogRef = this.dialog.open(EditGameComponent, dialogConfig);
    dialogRef.afterClosed().subscribe((result) => {
      // Handle the dialog close event here
      console.log('Dialog closed:', result);
    });
